                ...
                <div class="container">
                    <div class="row">
                        <div class="col-xs-8 col-sm-8 col-md-8 pull-left">
                            <h2 class="title title-d"> <strong> Receitas Prevista e Arrecadada </strong></h2>
                        </div>
<br>
                            ...
                            <div id="atualiza" class="pull-right hidden-xs">
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu" role="menu">
                                        <?php
                                        $sqlAnos = mysql_query("SELECT * FROM receitas WHERE Categoria = 'previsto' AND (CdPrefeitura = ".$CdPrefeitura.") GROUP BY Ano ORDER BY Ano DESC");
                                        $Anos = mysql_num_rows($sqlAnos);

                                        for ($y = 0; $y < $Anos; $y++){
                                            $verAnos = mysql_fetch_array($sqlAnos);
                                            ?>
                                            <li><a href="javascript:void(0)" onclick="carregaAnoReceita(<?php echo $verAnos['Ano'];?>,<?php echo $verAnos['CdPrefeitura']?>)"><?php echo $verAnos['Ano'];?></a></li>
                                        <?php
                                        }
                                        ?>
                                    </ul>
...
<section id="ver" class="texto-simples container">
    <div class="row">
        <?php
        $sqlLeis = mysql_query("SELECT * FROM receitas WHERE receitas = 'previsto' AND CdPrefeitura = '".$CdPrefeitura."' AND Ano = '" .$ano. "' ORDER BY Mes ASC");
            ...
            <div class="col-xs-6 col-md-4 ">
                <div class="item-para-download">
                    ...
                    <div class="link-de-download">
                        <p class="text-center">
                            <a class="btn type-d" title="" href="http://arquivo.minhaprefeitura.com.br/municipio/<?php echo $verPrefi['Pasta'];?>/anexo/<?php echo $verLeis['Arquivo'];?>" target="_blank">VIZUALIZAR</a>
                        </p>
                    </div>
                </div>
            </div>
