...
$sqlLinha = mysql_query("SELECT * FROM receitas WHERE (Categoria = 'previsto') AND (CdPrefeitura = '".$CdPrefeitura."') AND (Ano = '".$ano."')");
                ...
                <div id="atualiza" class="pull-right hidden-xs">
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            <?php
                            $sql3 = "SELECT * FROM receitas WHERE (Categoria = 'previsto') AND (Acao = 'Publicado') AND (CdPrefeitura = '".$CdPrefeitura."') GROUP BY Ano, Mes ORDER BY Ano DESC, Mes DESC";
...
<section id="ver" class="texto-simples container">
    <div class="row">
        <table class="table table-afiliados" summary="Despesas com pessoal">
            ...
            <tbody>
        <?php
        $sqlLeis = mysql_query("SELECT * FROM receita WHERE (tipo = 'prevista') AND (Acao = 'Publicado') AND (CdPrefeitura = '".$CdPrefeitura."') AND (ano = '".$ano."') ORDER BY data DESC");
        ...
        for ($y = 0; $y < $Leis; $y++){
            ...
            ?>
            <tr>
                <td><?php echo retorna_mes_extenso($verLeis['mes']);?>/<?php echo $verLeis['ano'];?></td>
                <td><?php echo $verLeis['codigo'];?></td>
                <td><?php echo $verLeis['especificacao'];?></td>
                <td><?php echo number_format($verLeis['previsao'], 2, ',', '.');?></td>
                <td><?php echo number_format($verLeis['arrecadado'], 2, ',', '.');?></td>
                <td><?php echo number_format($verLeis['realizado'], 2, ',', '.');?></td>
            </tr>
        <?php
        }
        ?>
