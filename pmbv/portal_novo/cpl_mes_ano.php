...
<?php
    ...
    $sqlUlt = mysql_query("SELECT * FROM cpl  WHERE Acao = 'Publicado' AND DATE_FORMAT(DtAbertura, '%c') = ".$SelMes." AND DATE_FORMAT(DtAbertura, '%Y') = ".$SelAno." GROUP BY DATE_FORMAT(DtAbertura, '%Y'), DATE_FORMAT(DtAbertura, '%m') ORDER BY DATE_FORMAT(DtAbertura, '%Y') DESC, DATE_FORMAT(DtAbertura, '%m') DESC LIMIT 1");
        ...
        <div class="row text-left">
            ...
            if ($Conta != 0) {
                ...
                <div id="anos" class="col-md-12">
                    ...
                    $sql = "SELECT * FROM cpl WHERE Acao = 'Publicado' AND DATE_FORMAT(DtAbertura, '%Y' )= " . $SelAno . " AND Acao <> 'Arquivo' AND Acao <> 'Correcao' GROUP BY DATE_FORMAT(DtAbertura, '%m') ORDER BY DATE_FORMAT(DtAbertura, '%Y') DESC, DATE_FORMAT(DtAbertura, '%m') DESC";
                    ...
                    for ($y = 0; $y < $Glossario; $y++) {
                        ...
                        if ($verGlossario['Acao'] == "Aguardando"){
                            ...
                            if (date('m', strtotime($verGlossario['DtAbertura'])) == $SelMes){
                ...
                <div id="verDados" class="col-md-12">
                    <div class="panel panel-default">
                        <div class="panel-body text-center">
                            <h4>
                                CONTRATOS E LICITAÇÕES DE <?php echo retorna_mes_extenso($SelMes); ?> / <?php echo $SelAno; ?>
                                <br>
                            ...
                            <?php
                            $sql = "SELECT * FROM cpl WHERE  Acao = 'Publicado' AND DATE_FORMAT(DtAbertura, '%m') = ".$SelMes." AND DATE_FORMAT(DtAbertura, '%Y') = ".$SelAno." ORDER BY Acao ASC";
                            $sqlGlossario = mysql_query($sql);
                            $Glossario = mysql_num_rows($sqlGlossario);
