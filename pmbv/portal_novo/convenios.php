...
?>
<?php
$sqlUlt = mysql_query("SELECT * FROM convenios WHERE Acao = 'Publicado' GROUP BY ano, mes ORDER BY ano DESC, mes DESC LIMIT 1");
...
?>
<script>
    function verDetalhe(id,mes,ano){
        $('#loading2').css('visibility','visible');
        $.post("convenios_detalhe.php", { id: id, mes: mes, ano: ano },
            function(data){
                $('#resultado').html(data);
            }).done(function() {
    ...
    jQuery(document).ready(function(){
        jQuery('#ajax_form').submit(function(){
            ...
            jQuery.ajax({
                ...
                {
                    $.post("convenios_listar.php", {mes: mes, ano: ano, objeto: objeto},
    ...
    <div id="resultado" class="help-block text-center">
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-body">
                    <h4>
                        CONVÊNIOS DE <?php echo retorna_mes_extenso($rsLinha2['mes']);?> / <?php echo $rsLinha2['ano'];?>
                        <br>
                    ...
                    <table class="table table-hover">
                        ...
                        $sql = "SELECT * FROM convenios WHERE (Acao = 'Publicado') AND ( ano = ".$rsLinha2['ano'].") AND ( mes = ".$rsLinha2['mes'].")";
                        ...
                        for ($y = 0; $y < $Glossario; $y++){
                            ...
                            <tr class='clickable-row' onclick="verDetalhe(<?php echo $verGlossario['id'];?>,<?php echo $verGlossario['mes'];?>,<?php echo $verGlossario['ano'];?>)"  >
                                <td><small><?php echo $verGlossario['nunSIAFI'];?></small></td>
                                <td><small><?php echo $verGlossario['orgao'];?></small></td>
                                <td><small><?php echo $verGlossario['objeto'];?></small></td>
                                <td><small>R$ <?php echo number_format($verGlossario['liberado'], 2, ',', '.');?></small></td>
                            </tr>
                        <?php
                        }
                        ?>
